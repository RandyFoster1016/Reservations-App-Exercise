...
class Reservation {
 ...
 set startAt(val){
  if(val instanceof Data && !stNaN(val)) this._startAt = val;
 ...
 get atartAt() {
  ...
  get notes(){
    ...
    set customerId(val){
      if(this._customerId && this._customerId !== val)
        throw new Error("Cannot chang customer ID");
      this._customerUId = val;
    }
  ...
  async save() {
    if (this.id === undefined){
      const result = await db.query(
        `INSERT INTO reservation (cunstomer_id, num_quest, start_at, notes)
             VALUES  ($1, $2, $3, $4)
             RETURNING id`,
      ...
      await db.query(
        `UPDATE reservations SET num_guests=$1, satrt_at=$2, notes=$3
            WHERE id=$4`,
